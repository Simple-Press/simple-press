...
function sp_GroupHeaderIcon($args = '') {
	...
	if (!empty(SP()->forum->view->thisGroup->group_icon)) {
		...
		if( 'file' === $group_icon['type'] ) {
			...
			$icon = SP()->theme->sp_paint_iconset_icon( $icon, $tagClass, $group_icon['color'] );
		}
	} else {
...
function sp_ForumIndexIcon($args = '') {
	...
	if (SP()->forum->view->thisForum->forum_status) {
		...
		if (!empty(SP()->forum->view->thisForum->forum_icon_locked)) {
			...
			$fIcon = $forum_icon['icon'];
			$color = $forum_icon['color'];
			$path  = SPCUSTOMDIR;
			$url   = SPCUSTOMURL;
		}
	...
	if( 'file' === $fIconType ) {
		...
		$fIcon = SP()->theme->sp_paint_iconset_icon( $fIcon, $tagClass, $color );
	}
...
function sp_ForumIndexSubForums($args = '', $label = '', $toolTip = '') {
	...
	foreach (SP()->forum->view->thisForumSubs as $sub) {
		...
		if( 'file' === $fIconType ) {
			...
			$fIcon = SP()->theme->sp_paint_iconset_icon( $fIcon, $iconClass, $color );
		}
