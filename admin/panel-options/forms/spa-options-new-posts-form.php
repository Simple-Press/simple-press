...
<script type="text/javascript">
    jQuery(document).ready(function() {
    	spjAjaxForm('sfnewpostsform', '');
        jQuery('#color-background').farbtastic('#flag-background');
        jQuery('#color-text').farbtastic('#flag-color');
    });
</script>
<?php
...
<?php
	...
	spa_paint_open_tab(spa_text('Options').' - '.spa_text('User New Posts Handling'));
		...
		spa_paint_open_panel();
			spa_paint_open_fieldset(spa_text('Users List of Unread Posts'), true, 'unread-posts');
		...
		spa_paint_open_panel();
			spa_paint_open_fieldset(spa_text('New/Unread Posts Flag Display'), true, 'new-post-flags');
				spa_paint_checkbox(spa_text('Display new post flags'), 'flagsuse', $sfoptions['flagsuse']);
				spa_paint_input(spa_text('Text to use in flags'), 'flagstext', $sfoptions['flagstext']);
			spa_paint_close_fieldset();
		spa_paint_close_panel();
...
?>
				<div>
					<span class="sfalignleft"><?php echo spa_text('New Post Flag background color'); ?>:</span>
				</div>
				<div>
				...
				<div>
					<span class="sfalignleft"><?php echo spa_text('New Post Flag text color'); ?>:</span>
				</div>
				<div>
					<input id="flag-color" type="text" value="#<?php echo $sfoptions['flagscolor']; ?>" name="flagscolor" style="width:100%;font-weight:bold;" />
					<div id="color-text" style="margin: 0px auto; width: 195px;"></div>
				</div>
<?php
    		spa_paint_close_fieldset();
    	spa_paint_close_panel();
...
?>
	...
	<input type="submit" class="button-primary" id="saveit" name="saveit" value="<?php spa_etext('Update New Post Handling'); ?>" />
	</div>
<?php
	spa_paint_close_tab();
?>
