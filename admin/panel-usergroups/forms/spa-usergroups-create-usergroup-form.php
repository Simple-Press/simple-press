...
?>
	<form action="<?php echo esc_attr($ajaxURL); ?>" method="post" id="sfusergroupnew" name="sfusergroupnew">
<?php
        echo '<input type="hidden" name="'.esc_attr('forum-adminform_usergroupnew').'" value="'.esc_attr(wp_create_nonce('forum-adminform_usergroupnew')).'" />';
		spa_paint_open_tab(/*SP()->primitives->admin_text('User Groups').' - '.*/SP()->primitives->admin_text('Create New User Group'), true);
			spa_paint_open_panel();
				spa_paint_open_fieldset(SP()->primitives->admin_text('Create New User Group'), 'true', 'edit-user-group');
					...
					spa_paint_select_end('<span class="sf-sublabel sf-sublabel-small">('.esc_html(SP()->primitives->admin_text('Upload badges on the Components - Forum Ranks admin panel')).')</span>');
