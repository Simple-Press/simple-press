...
?>
<script>
   	spj.loadAjaxForm('sfgroupedit<?php echo esc_js($group_id); ?>', 'sfreloadfb');
</script>
<?php
...
?>
	<form action="<?php echo esc_url($ajaxURL); ?>" method="post" id="sfgroupedit<?php echo esc_attr($group->group_id); ?>" name="sfgroupedit<?php echo esc_attr($group->group_id); ?>" class="sfinline-form">
<?php
...
?>
					...
					<input type="hidden" name="cgroup_name" value="<?php echo esc_attr(SP()->displayFilters->title($group->group_name)); ?>" />
					<input type="hidden" name="cgroup_desc" value="<?php echo esc_attr(SP()->editFilters->text($group->group_desc)); ?>" />
					...
					<input type="hidden" name="cgroup_rss" value="<?php echo esc_attr($group->group_rss); ?>" />
					<input type="hidden" name="cgroup_message" value="<?php echo esc_attr(SP()->editFilters->text($group->group_message)); ?>" />
<?php
					...
					spa_paint_input(SP()->primitives->admin_text('Replacement external RSS URL').'<br />'.SP()->primitives->admin_text('Default').': <strong>'.esc_url(SP()->spPermalinks->get_query_url(SP()->spPermalinks->build_url('', '', 0, 0, 0, 1)).'group='.$group->group_id).'</strong>', 'group_rss', SP()->displayFilters->url($group->group_rss), false, true);
			...
			spa_paint_open_panel();
				spa_paint_open_fieldset(SP()->primitives->admin_text('Default User Group Permissions'), false);
                    echo '<div class="sf-alert-block sf-info">';
                        echo '<strong>'.esc_html(SP()->primitives->admin_text('Set default usergroup permission sets for this group')).'</strong><br />';
                        SP()->primitives->admin_etext('Note - This will not will add or modify any current permissions. It is only a default setting for future forums created in this group.  Existing default usergroup settings will be shown in the drop down menus');
                    echo '</div>';
					# Permissions
					...
					foreach ($usergroups as $usergroup) {
						...
						foreach ($roles as $role) {
							...
							echo '<option '.esc_attr($selected).'value="'.esc_attr($role->role_id).'>'.esc_html(SP()->displayFilters->title($role->role_name)).'</option>'."\n";
						}
...
?>
		<div class="sf-form-submit-bar">
    		<input type="submit" class="sf-button-primary" id="groupedit<?php echo esc_attr($group->group_id); ?>" name="groupedit<?php echo esc_attr($group->group_id); ?>" value="<?php SP()->primitives->admin_etext('Update Group'); ?>" />
    		<input type="button" class="sf-button-primary spCancelForm" data-target="#group-<?php echo esc_attr($group->group_id); ?>" id="sfgroupedit<?php echo esc_attr($group->group_id); ?>" name="groupeditcancel<?php echo esc_attr($group->group_id); ?>" value="<?php SP()->primitives->admin_etext('Cancel'); ?>" />
		</div>
