...
?>
<script>
    spj.loadAjaxForm('sfforumedit<?php echo esc_attr($forum_id); ?>', 'sfreloadfb');
</script>
<?php
    global $tab;
...
<?php
        ...
        spa_paint_open_fieldset(SP()->primitives->admin_text('Forum Details'), false);
                    ...
                    echo "<input type='hidden' name='group_id' value='" . esc_attr($forum->group_id) . "' />";
                    echo "<input type='hidden' name='parent' value='" . esc_attr($forum->parent) . "' />";
                    echo "<input type='hidden' name='cgroup_id' value='" . esc_attr($forum->group_id) . "' />";
                    echo "<input type='hidden' name='cparent' value='" . esc_attr($forum->parent) . "' />";
                    echo "<input type='hidden' name='cchildren' value='" . esc_attr($forum->children) . "' />";
            ...
            //spa_paint_open_panel();
                spa_paint_open_fieldset(SP()->primitives->admin_text('Forum Options'), false);
                    ...
                    spa_paint_input(
                        SP()->primitives->admin_text('Replacement external RSS URL').'<br />'.SP()->primitives->admin_text('Default').': <strong>'.esc_html(SP()->spPermalinks->build_url($forum->forum_slug, '', 0, 0, 0, 1)).'</strong>',
                        'forum_rss',
                        esc_url(SP()->displayFilters->url($forum->forum_rss)),
                        false,
                        true
                    );

                    spa_paint_input(
                        SP()->primitives->admin_text('Custom meta keywords (SEO option must be enabled)'),
                        'forum_keywords',
                        esc_html(SP()->editFilters->text($forum->keywords)),
                        false,
                        true
                    );
                    spa_paint_wide_textarea(
                        'Special forum message to be displayed above forums',
                        'forum_message',
                        esc_html(SP()->editFilters->text($forum->forum_message))
                    );
				spa_paint_close_fieldset();
			//spa_paint_close_panel();
