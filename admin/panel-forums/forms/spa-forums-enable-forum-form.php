...
?>
<script>
   	spj.loadAjaxForm('sfforumenable<?php echo esc_attr($forum_id); ?>', 'sfreloadfb');
</script>
<?php
	spa_paint_options_init();
    $ajaxURL = wp_nonce_url(SPAJAXURL.'forums-loader&amp;saveform=enableforum', 'forums-loader');
?>
	<form action="<?php echo esc_url($ajaxURL); ?>" method="post" id="sfforumenable<?php echo esc_attr($forum_id); ?>" name="sfforumenable<?php echo esc_attr($forum_id); ?>">
<?php
		sp_echo_create_nonce('forum-adminform_forumenable');
...
?>
		<div class="sf-form-submit-bar">
    		<input type="submit" class="sf-button-primary" id="sfforumenable<?php echo esc_attr($forum_id); ?>" name="sfforumenable<?php echo esc_attr($forum_id); ?>" value="<?php SP()->primitives->admin_etext('Enable Forum'); ?>" />
    		<input type="button" class="sf-button-primary spCancelForm" data-target="#forum-<?php echo esc_attr($forum_id); ?>" id="sfforumenable<?php echo esc_attr($forum_id); ?>" name="enableforumcancel<?php echo esc_attr($forum_id); ?>" value="<?php SP()->primitives->admin_etext('Cancel'); ?>" />
    	</div>
