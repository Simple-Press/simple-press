...
function spa_toolbox_housekeeping_form() {
    ...
    <div id="sfhousekeepingformblock">
		...
		spa_paint_open_tab( esc_html( SP()->primitives->admin_text( 'House Keeping' ) ), true );
		spa_paint_open_panel();
                echo '<div class="sf-alert-block sf-warning">' . esc_html( SP()->primitives->admin_text( "Use options below with great caution." ) ) . '</div>';
		spa_paint_open_fieldset( esc_html( SP()->primitives->admin_text( 'Rebuild Indexes' ) ), true, 'rebuild-indexes' );
		?>
        <div class="collapsible-closed">
            ...
            <form action="<?php echo esc_url($ajaxURL); ?>" method="post" id="sfindexes" name="sfindexes">
                ...
                <input type="submit" class="sf-button-primary spShowElement" id="saveit1" name="rebuild-fidx" value="<?php SP()->primitives->admin_etext( 'Rebuild Forum Indexes' ); ?>"
                       data-target="#riimg"/>
                <img class="sfhidden" id="riimg" src="<?php echo esc_url( SPCOMMONIMAGES . 'working.gif' ); ?>" alt=""/>
            </form>
        </div>
        </div>
		<?php
		...
		spa_paint_open_panel();
		spa_paint_open_fieldset( esc_html( SP()->primitives->admin_text( 'New Post Cleanup' ) ), true, 'newpost-cleanup' );
		?>
        <div class="collapsible-closed">
            <div class="sf-form-row">
			<?php
			echo '<p class="sf-sublabel">' . esc_html( SP()->primitives->admin_text( 'This will reset the New Posts list for users who haven not visited the forum in the specified number of days.' ) ) . '</p>';
			?>
            <form action="<?php echo esc_url($ajaxURL); ?>" method="post" id="sfnewpostcleanup" name="sfnewpostcleanup">
                ...
                <img class="sfhidden" id="npcimg" src="<?php echo esc_url( SPCOMMONIMAGES . 'working.gif' ); ?>" alt=""/>
            </form>
            </div>
        </div>
		<?php
		...
		spa_paint_open_panel();
		spa_paint_open_fieldset( esc_html( SP()->primitives->admin_text( 'User Post Count Cleanup' ) ), true, 'post-count-cleanup' );
		?>
        <div class="collapsible-closed">
            <form action="<?php echo esc_url($ajaxURL); ?>" method="post" id="sfpostcountcleanup" name="sfpostcountcleanup">
            <div class="sf-form-row">
			<?php echo '<p class="sf-sublabel">' . esc_html( SP()->primitives->admin_text( 'This will go through the users and posts database tables and recalculate post counts for all users based on existing posts.' ) ) . '</p>'; 	?>
				<?php sp_echo_create_nonce( 'forum-adminform_housekeeping' ); ?>
                <p class="sf-sublabel .sf-sublabel-small">
					<?php echo esc_html( SP()->primitives->admin_text( 'Recalculating user post counts may take some time if you have a large number of users and cannot be reversed.' ) ); ?>
                </p>
                <input type="submit" class="sf-button-primary spShowElement" id="saveit3" name="postcount-cleanup" value="<?php SP()->primitives->admin_etext( 'Clean Up Post Counts' ); ?>"
                       data-target="#pcimg"/>
                <img class="sfhidden" id="pcimg" src="<?php echo esc_url( SPCOMMONIMAGES . 'working.gif' ); ?>" alt=""/>
		...
		spa_paint_open_panel();
		spa_paint_open_fieldset( esc_html( SP()->primitives->admin_text( 'Transient Cleanup' ) ), true, 'transient-cleanup' );
		?>
        <div class="collapsible-closed">
            <div class="sf-form-row">
			<?php
			echo '<p class="sf-sublabel">' . esc_html( SP()->primitives->admin_text( 'This will clean up expired WP Transients from the WP options table and any expired SP user notices.' ) ) . '</p>';
			?>
            <form action="<?php echo esc_url($ajaxURL); ?>" method="post" id="sftransientcleanup" name="sftransientcleanup">
                ...
                <input type="submit" class="sf-button-primary spShowElement" id="saveit4" name="transient-cleanup" value="<?php SP()->primitives->admin_etext( 'Clean Up Transients' ); ?>"
                       data-target="#tcimg"/>
                <img class="sfhidden" id="tcimg" src="<?php echo esc_url( SPCOMMONIMAGES . 'working.gif' ); ?>" alt=""/>
            </form>
        </div>
        </div>
		<?php
		...
		spa_paint_open_panel();
		spa_paint_open_fieldset( esc_html( SP()->primitives->admin_text( 'Rebuild Default Profile Tabs' ) ), true, 'reset-tabs' );
		?>
        <div class="collapsible-closed">
            <div class="sf-form-row">
			<?php
			echo '<p class="sf-sublabel">' . esc_html( SP()->primitives->admin_text( 'This will remove all Profile Tabs and restore to default state.' ) ) . '</p>';
			?>
            <form action="<?php echo esc_url($ajaxURL); ?>" method="post" id="sfresetprofiletabs" name="sfresetprofiletabs">
                ...
                <input type="submit" class="sf-button-primary spShowElement" id="saveit5" name="reset-tabs" value="<?php SP()->primitives->admin_etext( 'Reset Profile Tabs' ); ?>"
                       data-target="#rdptimg"/>
                <img class="sfhidden" id="rdptimg" src="<?php echo esc_url( SPCOMMONIMAGES . 'working.gif' ); ?>" alt=""/>
            </form>
        </div>
        </div>
		<?php
		...
		spa_paint_open_panel();
		spa_paint_open_fieldset( esc_html( SP()->primitives->admin_text( 'Reset the Auths Cache' ) ), true, 'reset-auths' );
		?>
        <div class="collapsible-closed">
            <div class="sf-form-row">
			<?php
			echo '<p class="sf-sublabel">' . esc_html( SP()->primitives->admin_text( "This will force a rebuild of each user's auth cache. It does not change any permissions." ) ) . '</p>';
			?>
            <form action="<?php echo esc_url($ajaxURL); ?>" method="post" id="sfresetauths" name="sfresetauths">
                ...
                <input type="submit" class="sf-button-primary spShowElement" id="saveit6" name="reset-auths" value="<?php SP()->primitives->admin_etext( 'Reset Auths Cache' ); ?>"
                       data-target="#rtacimg"/>
                <img class="sfhidden" id="rtacimg" src="<?php echo esc_url( SPCOMMONIMAGES . 'working.gif' ); ?>" alt=""/>
            </form>
        </div>
        </div>
		<?php
		...
		spa_paint_open_panel();
		spa_paint_open_fieldset( esc_html( SP()->primitives->admin_text( 'Reset Users Plugin Data Cache' ) ), true, 'reset-plugin-data' );
		?>
        <div class="collapsible-closed">
            <div class="sf-form-row">
			<?php
			echo '<p class="sf-sublabel">' . esc_html( SP()->primitives->admin_text( "This will force each user's plugin data cache to be cleared." ) ) . '</p>';
			?>
            <form action="<?php echo esc_url($ajaxURL); ?>" method="post" id="sfresetplugdata" name="sfresetplugdata">
                ...
                <input type="submit" class="sf-button-primary spShowElement" id="saveit10" name="reset-plugin-data" value="<?php SP()->primitives->admin_etext( 'Reset Users Plugin Data' ); ?>"
                       data-target="#rrpdimg"/>
                <img class="sfhidden" id="rrpdimg" src="<?php echo esc_url( SPCOMMONIMAGES . 'working.gif' ); ?>" alt=""/>
            </form>
            </div>
        </div>
		<?php
		...
		spa_paint_open_panel();
		spa_paint_open_fieldset( esc_html( SP()->primitives->admin_text( 'Reset combined CSS/JS' ) ), true, 'reset-combined' );
		?>
        <div class="collapsible-closed">
            <div class="sf-form-row">
			<?php
			echo '<p class="sf-sublabel">' . esc_html( SP()->primitives->admin_text( "This will force a rebuild of the combined CSS and JS cache files." ) ) . '</p>';
			?>
            <form action="<?php echo esc_url($ajaxURL); ?>" method="post" id="sfresetcombined" name="sfresetcombined">
                ...
                <img class="sfhidden" id="rtccimg" src="<?php echo esc_url( SPCOMMONIMAGES . 'working.gif' ); ?>" alt=""/>
            </form>
            </div>
        </div>
		<?php
		...
		spa_paint_open_panel();
		spa_paint_open_fieldset( esc_html( SP()->primitives->admin_text( 'Flush general cache' ) ), true, 'flush-cache' );
		?>
        <div class="collapsible-closed">
            <div class="sf-form-row">
			<?php
			echo '<p class="sf-sublabel">' . esc_html( SP()->primitives->admin_text( "This will force a flushing of the general cache." ) ) . '</p>';
			?>
            <form action="<?php echo esc_url($ajaxURL); ?>" method="post" id="sfflushcache" name="sfflushcache">
                ...
                <img class="sfhidden" id="fcacheimg" src="<?php echo esc_url( SPCOMMONIMAGES . 'working.gif' ); ?>" alt=""/>
            </form>
            </div>
        </div>
		<?php
		...
		spa_paint_open_panel();
		spa_paint_open_fieldset( esc_html( SP()->primitives->admin_text( 'Flush XML cache' ) ), true, 'flush-xml' );
		?>
        <div class="collapsible-closed">
            <div class="sf-form-row">
			<?php
			echo '<p class="sf-sublabel">' . esc_html( SP()->primitives->admin_text( "This will force a flushing of the xml api cache." ) ) . '</p>';
			?>
            <form action="<?php echo esc_url($ajaxURL); ?>" method="post" id="sfflushxml" name="sfflushxml">
                ...
                <img class="sfhidden" id="fcachexml" src="<?php echo esc_url( SPCOMMONIMAGES . 'working.gif' ); ?>" alt=""/>
            </form>
            </div>
        </div>
		<?php
