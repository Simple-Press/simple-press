	...
	spa_paint_open_panel();
		echo '<div class="helpAndFAQ">';
			spa_paint_open_fieldset(SP()->primitives->admin_text('Task Glossary'), false);
				echo '<p><b>'.esc_html(SP()->primitives->front_text("Select the item to display tasks for from the list below")).'</b></p>';
				...
				if ($keywords) {
					...
					foreach ($keywords as $keyword) {
						...
						echo "<li class='key-word' data-url='".esc_url($ajaxPostURL)."'>";
						echo esc_html($keyword->keyword).'</li>';
					}
					echo '</ul>';
				}
	...
	spa_paint_open_panel();
		echo '<div class="helpAndFAQ">';
			spa_paint_open_fieldset(SP()->primitives->admin_text('Associated Tasks'), false);
				echo "<div id='codex' class='codex'>".esc_html(SP()->primitives->admin_text('No current selection from the Task Glossary'))."</div>";
			spa_paint_close_fieldset();
		echo '</div>';
	spa_paint_close_panel();
spa_paint_close_tab();
