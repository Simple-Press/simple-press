...
?>
            <div class="sf-alert-block sf-info">
                <p><?php esc_html_e(SP()->primitives->admin_text('Upload a Simple:Press plugin in .zip format')); ?></p>
                <p><?php esc_html_e(SP()->primitives->admin_text('If you have a plugin in a .zip format, you may upload it here')); ?></p>
            </div>
            <form method="post" enctype="multipart/form-data" action="<?php esc_attr_e(self_admin_url('update.php?action=upload-sp-plugin')); ?>" id="sfpluginuploadform" name="sfpluginuploadform">
                <?php echo '<input type="hidden" name="'.esc_attr().'" value="'.esc_attr(wp_create_nonce()).'" />'; ?>
				<div class="clear"></div>
                ...
                <label class="sf-button-primary sf-upload-button">
                    ...
                    <?php echo esc_html(SP()->primitives->admin_text('Select file')); ?>
                </label>
                ...
                <div id="sf-loader-gif" style="display: none;">
                    <img src="<?php esc_attr_e(SPADMINIMAGES . 'sp_WaitBox.gif'); ?>" alt="loading" />
                </div>
            </form>
            <script>
                function sfloader(){
