...
?>
            <div class="sf-alert-block sf-info">
                <p><?php echo esc_html(SP()->primitives->admin_text('Upload a Simple:Press plugin in .zip format')); ?></p>
                <p><?php echo esc_html(SP()->primitives->admin_text('If you have a plugin in a .zip format, you may upload it here')); ?></p>
            </div>
            <form method="post" enctype="multipart/form-data" action="<?php echo esc_attr(self_admin_url('update.php?action=upload-sp-plugin')); ?>" id="sfpluginuploadform" name="sfpluginuploadform">
                <?php echo '<input type="hidden" name="'.esc_attr('forum-plugin_upload').'" value="'.esc_attr(wp_create_nonce('forum-plugin_upload')).'" />'; ?>
				<div class="clear"></div>
