...
function sp_update_wp_counts($counts, $titles) {
	...
	if (!empty($pup) || !empty($tup)) {
		if (!empty($pup->response)) {
			...
			$titles['sp_plugins'] = sprintf(_n('%d Simple:Press Plugin Update', '%d Simple:Press Plugin Updates', esc_html($num)), esc_html($num));
		}
		if (!empty($tup->response)) {
			$num = count($tup->response);
			$counts['counts']['themes'] = $counts['counts']['themes'] + $num;
			$titles['sp_themes'] = sprintf(_n('%d Simple:Press Theme Update', '%d Simple:press Theme Updates', esc_html($num)), esc_html($num));
		}
